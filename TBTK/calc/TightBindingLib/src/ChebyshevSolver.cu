...
void multiplyMatrixAndVector(cuDoubleComplex *jIn,
	...
	if(to < basisSize)
		for(int n = 0; n < maxHoppingAmplitudes; n++)
			jResult[to] = cuCadd(jResult[to], cuCmul(damping[to], cuCmul(hoppingAmplitudes[maxHoppingAmplitudes*to + n], jIn[fromIndices[maxHoppingAmplitudes*to + n]])));
